...
public class Dijkstra {
  ...
  Collection<Vertex> cityCollection;
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
        // pythag thm
	  	double sideASquared = Math.pow((ux - vx), 2);
	  	double sideBSquared = Math.pow((uy - vy), 2);
	  	return Math.pow((sideASquared + sideBSquared), 0.5);
  ...
  public void computeAllEuclideanDistances() {
	  //get cities
	  cityCollection = getVertices();
	  //loop through adjacent edges of the city
	  for(Vertex adjEdges: cityCollection){
		  List<Edge> cityEdges = adjEdges.adjacentEdges;
		  //set eudlidean distance for each edge
  ...
  public void doDijkstra(String s) {
      cityCollection = getVertices();
      //list for vertices that still need to be done
	  LinkedList<Vertex> verticesLeft = new LinkedList<>();
	  //for each vertex, set distance and known (as in dijkstra's algorithm in weiss)
	  for (Vertex vertex: cityCollection){
		  vertex.distance = Double.POSITIVE_INFINITY;
		  vertex.known = false;
		  //add each vertex to the list of vertices that still need to be done
		  verticesLeft.add(vertex);
	  }
	  //set distance for the first city to zero (as in dijkstra's algorithm in weiss)
	  getVertex(s).distance = 0;
	  //loop through the verticesLeft list
	  while(!(verticesLeft.isEmpty())){
		  Vertex nextCity = verticesLeft.get(0);
		  //find smallest unknown distance vertex (as in dijkstra's algorithm in weiss)
		  //loop through the verticesLeft, find the smallest distance for w not known:
		  
		  for (Vertex currentCity: verticesLeft){
			  if (nextCity.distance > currentCity.distance){
				  nextCity = currentCity;
			  }
		  }
		  //set the known value to true and move to the next city 
		  nextCity.known = true;
		  
		  //for each vertex w adjacent to v (as in dijkstra's algorithm in weiss)
		  //go through the edges to get the edge path:
		  for (Edge edge: nextCity.adjacentEdges){
			  //if w not known as in weiss...
			  if (!(edge.target.known)){
		  		  if ((edge.source.distance + edge.distance) < edge.target.distance) {
				      //update w	  
          			  edge.target.distance = edge.source.distance + edge.distance;
          			  edge.target.prev = edge.source;
      			}
			  }
		  }
		  verticesLeft.remove(nextCity);
	  }
  }
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
      doDijkstra(s);
      Vertex dest = getVertex(t);
      Vertex end = dest;
      LinkedList<Edge> path = new LinkedList();
      //runs through the linked list backwards until reaching the 
      //head node
      while(end.prev != null){
    	  double distance = computeEuclideanDistance(end.prev.x, end.prev.y, end.x, end.y);
    	  path.addFirst(new Edge(end.prev, end, distance));
    	  end = end.prev;
      }
	  return path; 
  }
